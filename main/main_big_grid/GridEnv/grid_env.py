...
class grid_environment():
    def __init__(self, epi_length_limit = 20000):
        mdppath = './GridEnv/mdp_big.txt'
    ...
    def step(self, action):
        ...
        # Adding stochasticity of 10% to transisitions (Noise in generative process trans. B)
        noise_transition = np.random.choice([-1,0,+1], size = None, replace = True, p = [0.05, 0.9, 0.05])
        noise_transition = 0 if n_s == (0 or self.numS-1) else noise_transition
        
        n_s = n_s + noise_transition
