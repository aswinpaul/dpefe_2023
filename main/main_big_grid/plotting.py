...
def plot_result(data):
    ...
    plt.fill_between(range(sample-1), ql_rm_3[:-1] + 0.5*ql_rs_3[:-1], 0.5*ql_rm_3[:-1] - ql_rs_3[:-1], alpha=0.3)
    plt.fill_between(range(sample-1), ql_rm_4[:-1] + ql_rs_4[:-1], ql_rm_4[:-1] - ql_rs_4[:-1], alpha=0.3)
    plt.fill_between(range(sample-1), ql_rm_5[:-1] + 0.5*ql_rs_5[:-1], ql_rm_5[:-1] - 0.5*ql_rs_5[:-1], alpha=0.3)
    
    plt.legend(["Q-Learning", "Dyna-Q (replay=10)", "SI agent", "DPEFE agent", "Random agent"])
