...
plt.fill_between(range(sample-1), np.clip(ql_rm_2[:-1] + ql_rs_2[:-1], 0, 7900), np.clip(ql_rm_2[:-1] - ql_rs_2[:-1], 0, 999), alpha=0.3)
plt.fill_between(range(sample-1), np.clip(ql_rm_3[:-1] + ql_rs_3[:-1], 0, 7900), np.clip(ql_rm_3[:-1] - ql_rs_3[:-1], 0, 999), alpha=0.3)
plt.fill_between(range(sample-1), np.clip(ql_rm_4[:-1] + ql_rs_4[:-1], 0, 7900), np.clip(ql_rm_4[:-1] - ql_rs_4[:-1], 0, 999), alpha=0.3)
...
plt.title("Deterministic grid environment (50 states)")

plt.xlabel("Episode number in trial")
plt.ylim(0, 5000)
plt.xlim(0, 50)
plt.ylabel("Length of epsiode")
plt.savefig('perf_smallgrid_d.png', dpi=500, bbox_inches='tight');
