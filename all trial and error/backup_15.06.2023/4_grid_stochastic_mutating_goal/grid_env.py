...
class grid_environment():
    ...
    def __init__(self, path = "grid20.txt", epi_length_limit = 10000):
    ...
    def reset(self, seed = 10, randomise_goal = False):

        self.startS = np.random.randint(0,self.numS)
        if(randomise_goal):
            self.end_state = np.random.randint(0,self.numS)

        while(self.startS == self.end_state):
            self.startS = np.random.randint(0,self.numS)
            if(randomise_goal):
                self.end_state = np.random.randint(0,self.numS)

        self.T[self.end_state,:,self.end_state] = 1
