...
class agent():
    ...
    def plan_tree_search(self, modalities = False):
        ...
        for mod in moda:
            N = 0
            #self.countt = 0
            self.G += self.forward_search(mod, self.Q_actions, N) 
            
            #print(self.countt)
            
        # Distribution for action-selection
        for l in range(self.numS):
            self.Q_actions[:,l] = softmax(-1*self.action_precision*self.G[:,l])
                
    def forward_search(self, mod, Q_actions, N):
        # self.countt += 1
        N += 1
        ...
        for i in range(self.numS):
            for j in range(self.numA):
                ...
                G[j,i] += kl_div(Q_po[:,i,j],self.C[mod]) + np.dot(self.B[0][:,i,j],entropy(self.A[mod]))
                if(Q_actions[j,i] > self.threshold):
                    x = np.sum(np.matmul(np.reshape(np.multiply(Q_actions[:,:],self.forward_search(mod, Q_actions, N)), (self.numA,self.numS)),np.reshape(self.B[0][:,i,j], (self.numS,1))))
                    G[j,i] += x
                                                      
        # Distribution for action-selection
        for l in range(self.numS):
            Q_actions[:,l] = softmax(-1*G[:,l])
    ...
    def take_decision(self):
        ...
        p = np.matmul(self.Q_actions, self.qs[0])
    ...
    def update_c(self, prev_obs, obs, reward, moda = False, factor = 1):
        if(moda == False):
            for mod in range(self.num_modalities):
                exp_reward = np.clip(np.exp(reward), 0, 1000)
                if(self.c[mod][prev_obs[mod]] < 10000):
                    self.c[mod][prev_obs[mod]] += exp_reward*self.c[mod][obs[mod]]
