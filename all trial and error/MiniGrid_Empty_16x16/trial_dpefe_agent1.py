...
from agent_dpefe_learnc import agent as dpefe_agent
...
s1_size = 256 # (NoofStates times NoofDirection-of-agent)
...
for sl in range(seedloops):
    ...
    a = dpefe_agent(num_states, num_obs, num_controls, a = A,
                    planning_horizon = N, action_precision = 10, MDP = True)
    ...
    for ts in range(episodes):
        ...
        if((ts == 5 or ts == 10 or ts == 15) and seen_goal == True):
            a.plan_using_dynprog(modalities=[0])
            planning_count += 1
        ...
        while(done == False):
            ...
            a.update_c(prev_obs, obs, reward, moda=[0])
            seen_goal = True
        ...
        data[sl][ts] = score
        a.end_of_trial(learning = True, factor = 100)
