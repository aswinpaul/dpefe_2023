...
with open('data_trial_zl.npy', 'rb') as g:
        data_zl = np.load(g)
        
with open('data_trial_random.npy', 'rb') as g:
        data_random = np.load(g)
...
def plot_result(data):
    ...
    ql_rs_5 = np.std(np.transpose(data[4]), axis=1)
    ...
    plt.fill_between(range(sample-1), np.clip(ql_rm_5[:-1] + ql_rs_5[:-1], None, x_max), np.clip(ql_rm_5[:-1] - ql_rs_5[:-1], x_min, None), alpha=0.3)
    
    plt.legend(["Q-Learning", "Dyna-Q (replay=10)", "DPEFE Agent", "Z-Learning", "Random"])
    plt.title("Agents in 'MiniGrid-Empty-5x5' Environment")
...
data_for_plotting[3,:,:] = data_zl[:,:]

data_for_plotting[4,:,:] = data_random[:,:]
