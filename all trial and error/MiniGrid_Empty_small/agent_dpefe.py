...
class agent():
    ...
    def __init__(self, num_states, num_obs, num_controls, planning_horizon, 
        ...
        self.A = random_A_matrix(self.num_obs, self.num_states)*0 
        self.B = random_B_matrix(self.num_states, self.num_controls)*0
            ...
            for k in range(T-2,-1,-1):
                for i in range(self.numA):
                    for j in range(self.numS):
                        ...
                        if(k == T-2):
                            self.G[k,i,j] += self.T*kl_div(Q_po[:,j,i],self.C[mod]) + np.dot(
