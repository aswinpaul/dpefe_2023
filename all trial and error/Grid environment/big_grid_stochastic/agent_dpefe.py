...
class agent():
    ...
    def plan_using_dynprog(self, modalities=False):
        ...
        for mod in moda:
            Q_po = np.einsum('ij,klj->kli', self.A[mod], self.B[0])

            for k in range(T - 2, -1, -1):
                kl_term = kl_div(Q_po[:, :, k, :], self.C[mod])
                entropy_term = np.dot(self.B[0][:, :, k, :], entropy(self.A[mod]))
                self.G[k, :, :] += kl_term + entropy_term

                QG_prod = np.einsum('ijk,jk->ik', self.Q_actions[k + 1, :, :], self.G[k + 1, :, :])
                self.G[k, :, :] += np.matmul(QG_prod, self.B[0][:, :, k, :])

            self.Q_actions[:-1, :, :] = softmax(-1 * self.planning_precision * self.G[:-1, :, :])
        ...
        #for mod in moda:
            ...
            #for k in range(T-2,-1,-1):
                ...
                #for l in range(self.numS):
                    #self.Q_actions[k,:,l] = softmax(-1*self.planning_precision*self.G[k,:,l])
