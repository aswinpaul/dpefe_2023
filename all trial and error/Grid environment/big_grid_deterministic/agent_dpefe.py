    ...
    def plan_using_dynprog(self, modalities=False):
        ...
        for mod in moda:
            ...
            for j in range(self.numA):
                Q_po[:, :, j] = A_mod.dot(B_0[:, :, j])
            
            G_k = self.G[T - 2]
            G_kp1 = self.G[T - 2]
            Q_actions_k = self.Q_actions[T - 2]
            
            for k in range(T - 2, -1, -1):
                G_kp1 = G_k
                G_k = self.G[k]
                Q_actions_kp1 = Q_actions_k
                Q_actions_k = self.Q_actions[k]
                Q_po_kp1 = Q_po
                
                for i in range(self.numA):
                    G_k[i, :] += kl_div(Q_po_kp1[:, :, i], C_mod) + np.dot(B_0[:, :, i], entropy(A_mod))
                    G_k[i, :] += np.sum(np.matmul(np.multiply(Q_actions_kp1, G_kp1), B_0[:, :, i].T), axis=0)
                
                Q_actions_k[:] = softmax(-self.planning_precision * G_k)
