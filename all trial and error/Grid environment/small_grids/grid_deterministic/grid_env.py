...
class grid_environment():
    def __init__(self, path = "grid.txt", epi_length_limit = 10000):
        
        os_comm = str("python3 encoder.py > mdp.txt")
        os.system(os_comm)
        mdppath = 'mdp.txt'
    ...
    def render(self, path="grid10.txt"):
    ...
    def render_c_matrix(self, path="c_matrix.txt"):
        plt.figure(figsize=(2, 2))
        grid = np.loadtxt(path, dtype = int)
        plt.imshow(grid, cmap=plt.cm.CMRmap, interpolation='nearest') #
        plt.xticks([]), plt.yticks([])
