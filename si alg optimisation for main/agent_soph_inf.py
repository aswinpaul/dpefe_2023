...
class agent():
    ...
    def __init__(self, num_states, num_obs, num_controls, planning_horizon = 2, 
                 episode_horizon = 100,
                 a = 0, b = 0, c = 0, d = 0, action_precision = 16, 
                 planning_precision = 16, MDP = False, 
        ...
        for i in range(self.numS):
            for j in range(self.numA):
                ...
                if(N < self.N and (self.Q_actions[j,i] > self.threshold)):
                    ...
                    # np.sum(np.matmul(np.reshape(np.multiply(
                    #     self.Q_actions[k+1,:,:],self.G[k+1,:,:]), 
                    #     (self.numA,self.numS)),np.reshape(self.B[0][:,j,i], 
                    #                                       (self.numS,1))))
                                                          
                    a = np.reshape(np.multiply(self.Q_actions[:,:], G_next), (self.numA,self.numS))
    ...
    def update_c(self, prev_obs, obs, reward, moda = False, factor = 1, terminal = False):
        if(moda == False):
