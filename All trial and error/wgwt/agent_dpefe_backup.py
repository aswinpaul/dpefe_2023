...
class agent():
    ...
    def __init__(self, num_states, num_obs, num_controls, planning_horizon, 
                 a = None, b = None, c = None, d = None, action_precision = 1,
                 initial_planning = False, planning_every_time_step = False, MDP = False):
        ...
        self.MDP = MDP
        self.planning_every_time_step = planning_every_time_step

        if(initial_planning == True):
    ...
    def infer_hiddenstate(self, obs):
        ...
        for i in range(len(self.num_states)):
            ...
            if(self.MDP == True):
                ...
                self.qs[i] = softmax(100*(term_1 + term_2))
        ...
        for mod in moda:
            ...
            for k in range(T-2,-1,-1):
                ...
                for l in range(self.numS):
                    self.Q_actions[k,:,l] = softmax(-1*self.G[k,:,l])
                    
    # Decision making
    def take_decision(self,tau):
        
        tau = 0 if tau > self.T-2 else tau
        
        pp = np.matmul(self.Q_actions[tau], self.qs[0])
        p = softmax(self.action_precision * pp)
    ...
    def update_c(self, obs):
        for mod in range(self.num_modalities):
            self.c[mod] += onehot(obs[mod], self.num_obs[mod])
    ...
    def learn_parameters(self):
        for i in range(self.num_modalities):
            for k in range(self.num_states[0]):
                self.A[i][:,k] = dirichlet.mean(self.a[i][:,k] + self.EPS_VAL)
        
        for i in range(len(self.num_states)):
            for j in range(self.num_states[i]):
                for k in range(self.num_controls[i]):
                    self.B[i][:,j,k] = dirichlet.mean(self.b[i][:,j,k] + self.EPS_VAL)
                
        for i in range(len(self.num_states)):
            self.D[i] = softmax(100*self.d[i])
            
        for mod in range(self.num_modalities):
            self.C[mod] = softmax(100*self.c[mod])
    ...
    def step(self, obs_list, tau):
        ...
        if(tau == 0):
            ...
            # Planning
            if(self.planning_every_time_step == True):
                self.plan_using_dynprog()
            
            # Decision making
            action = self.take_decision(tau)
